...
def swing(speed=0):
    ...
    play_sound()
    # speed = speed / 10
    # if speed > MAXIMUM_SWING_SPEED:
    #     speed = MAXIMUM_SWING_SPEED

    # ratio = speed / 4
    # tip_pixel_color = (
    #     int(COLOR[0] + (COLOR_DIFFERENCE[0] * ratio)),
    #     int(COLOR[1] + (COLOR_DIFFERENCE[1] * ratio)),
    #     int(COLOR[2] + (COLOR_DIFFERENCE[2] * ratio)),
    # )
    # STRIP.set_pixel_line_gradient(
    #     PIXEL_COUNT - 1, PIXEL_COUNT - 1 - SWING_PIXEL_COUNT, COLOR, tip_pixel_color
    # )
    # if IS_TURNED_ON:
    #     STRIP.show()
...
while True:
    ...
    if gyro_input[1] < -4.3 and abs(gyro_input[0]) < 1.5 and abs(gyro_input[2] < 1.5):
        print("turn detected")
